...
router.delete('/deleteStaff', (req, resp) => {
...
router.post('/addAdmin',(req,resp)=>{
    ...
    const{ name, email,password}=req.body
    const encrpPass = cryptojs.MD5(password)
    const query =` insert into admin(name, email, password) values('${name}', '${email}', '${encrpPass}')`
})
...
router.post('/signin', (req, resp) => {
    ...
    const query = `select name, email from admin where email = '${email}' and password='${encrpPass}'`
